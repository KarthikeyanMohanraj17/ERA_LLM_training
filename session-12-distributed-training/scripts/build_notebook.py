...
md(r"""
**All four still train.** "The loss went down" proves nothing.
...
- At world=1 it agrees with DDP bitwise, so its sharding works.
- At world=4 it disagrees — but **PyTorch's own DDP and FSDP2 disagree by the same amount**, so the gap isn't on our side.
- Cause: FSDP2 hooks its gradient sync onto the wrapped module's **input**. This model's input is integer token IDs, which carry no gradient, so the hook never fires.

**What can be claimed:** ZeRO-0 is verified against PyTorch DDP. ZeRO-1/2/3 are verified against a single-GPU reference and each other — **not** against FSDP.
""")
